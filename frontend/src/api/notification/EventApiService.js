...
export function createEvent(key, title, date, time, description, invites, notify, type, status) {
  const email = localStorage.getItem("email");
  const jwtToken = localStorage.getItem("jwtToken");
  ...
  return eventClient.post(
    ...
    { key:key, owner:email, member:email, title:title, date:date, time:time, description:description, invites:invite, timestamp:unixTimestampMillis, type:type, notify:notify, status:status}, // Pass the message in the request payload
    {
      headers: {
        Authorization: jwtToken,
        "Content-Type": "application/json",
...
export function pushEvent(key, owner, title, date, time, description, invites, notify, type, status) {
  const email = localStorage.getItem("email");
  const jwtToken = localStorage.getItem("jwtToken");
